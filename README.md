...
### Wan — Emotion, Equity & Recommendation Agents

| Area | Details |
|------|---------|
| Equity Agent | LightGBM salary regression model, BLS market benchmark integration (`agents/equity/`) |
| Emotion Agent | Sentence-Transformers (all-MiniLM-L6-v2) embedding, RoBERTa sentiment classification, Glassdoor NLP pipeline, Claude sentiment report (`agents/emotion/`, `tools/emotion_tool.py`) |
| Recommendation Agent | Claude tool-use agentic loop — `get_employee_profiles` → `save_recommendations`; weighted per-employee retention plans with HR/Management routing (`agents/recommendation/`) |
| LangGraph Pipeline (shared) | Graph topology design, node wiring, fan-in/fan-out structure, conditional audit loop (`agents/pipeline/pipeline.py`) |
| Scheduled Pipeline | GitHub Actions cron workflow — monthly automated pipeline run (`/.github/workflows/pipeline.yml`) |
| MongoDB Schema | Collection design for Equity_Predictions, Emotion, retention_recommendations |

### Yvony — Retention, Audit & Data Infrastructure

| Area | Details |
|------|---------|
| Retention Agent | Cox Proportional Hazard survival model, salary risk scoring, Claude HR analysis (`agents/retention/`) |
| Recommendation Audit Agent | Adversarial Claude critic — quality scoring, rule checks, revision instruction generation (`agents/recommendation_audit/`) |
| LangGraph Pipeline (shared) | State management, retry logic, `_should_continue` conditional edge, pipeline integration (`agents/pipeline/pipeline.py`) |
| FastAPI Backend | `/api/login` and `/api/employees` endpoints, role-based filtering, MongoDB query logic (`api/server.py`) |
| Mock Data & Deployment | Employee dataset generation, MySQL seeding, GitHub Pages + Render deployment, CORS configuration (`scripts/`) |

### AI-Generated — Frontend

The React frontend (`retention-ui/src/App.jsx`) was fully generated with the assistance of Claude (Anthropic). This includes the complete single-page application — Home, Login, Portal, HR Dashboard, and Manager View — along with all UI components, role-based routing, API integration, and visual design.
...
Add pytest-based unit tests covering the core ML scoring logic (Cox risk bucketing, salary gap calculation), LangGraph node outputs, and API endpoint responses to ensure pipeline reliability across monthly runs.

### 2. RAG Integration

Integrate a Retrieval-Augmented Generation layer so the Recommendation Agent can ground its retention plans in company-specific HR policy documents and industry benchmarks, improving specificity beyond what employee profile data alone provides.

### 3. Replace mock data with live MySQL data
...
### 4. Migrate user authentication to MySQL
...
The planned upgrade is to store all user accounts, hashed passwords, roles, and department assignments in a `users` table in MySQL, enabling dynamic account management without touching the source code.
